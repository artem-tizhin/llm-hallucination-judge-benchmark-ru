...
│
├── analysis/
│ └── benchmark_metrics_analysis.ipynb # ноутбук с анализом метрик
│
├── dataset/
│ └── questions.jsonl # датасет с вопросами
│
├── evaluation/
│ ├── init.py
│ └── evaluate.py # подсчет метрик по результатам
│
├── metrics/
│ ├── init.py
│ └── score.py # функции метрик (accuracy, и др.)
│
├── models/
│ ├── init.py
│ ├── benchmark_runner.py # общий пайплайн запуска бенчмарка
│ ├── judge.py # логика модели-судьи (LLM-as-a-judge)
...
│
├── prompts/
│ ├── answer_prompt.txt # промпт для генерации ответа моделью
│ └── judge_prompt.txt # промпт для judge-модели
│
├── results/
